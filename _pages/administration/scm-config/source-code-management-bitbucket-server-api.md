...
See the [Bitbucket Server (build-on-checkin)](/administration/scm-config/source-code-management-bitbucket-server-checkin) page for details on how to set up a configuration with the other provider type.
...
![]({{site.url}}/images/post_images/algo-images-admin/algo-1626195883283.png)
...
![]({{site.url}}/images/post_images/algo-images-admin/algo-1626211316100.png)
...
The `Authorization` header displayed in the sample code on this page is listed as _`Simple|Bearer`_ to indicate that either an [API key](https://algorithmiaio.github.io/platform/customizing-api-keys) or a [JSON Web Token (JWT)](https://algorithmiaio.github.io/platform/jwt-authentication) can be used as an authentication token. Switching between these two authentication modes simply involves switching the `Authorization` header between `Simple` type and `Bearer` type, and providing the corresponding authentication token. All other aspects of the REST calls shown here are unaffected by this header.

In this flow, two different Algorithmia user roles are required, and the authentication tokens are represented as follows:

*   `CLUSTER_ADMIN_AUTH_TOKEN` represents either an [admin API key](https://algorithmiaio.github.io/platform/customizing-api-keys#admin-api-keys) with [Management Access](https://algorithmiaio.github.io/algorithm-development/algorithm-management) turned on or a JWT of a user who has cluster admin access rights on the target Algorithmia cluster.
*   `ORG_ADMIN_AUTH_TOKEN` represents either a standard (non-admin) API key with [Management Access](https://algorithmiaio.github.io/algorithm-development/algorithm-management) turned on, generated by a user with admin rights to the org that owns a given algorithm, or a JWT of a user who has org admin access rights in the target org on the Algorithmia cluster.
